...
const CountriesGridList: React.FC = () => {
  ...
  const handleAddToFavorite = (country: Country) => {
    if (isCountryFavorite(country)) {
      ...
      toast.success(
        `${country.name.official} has been added to your favoriets`
      );
    }
  };
  ...
  const isCountryFavorite = (country: Country) => {
    return favoriteCountries.some(
      favoriteCountry => favoriteCountry.name.official === country.name.official
    );
  };
                ...
                <TableRow key={uuidv4()}>
                  ...
                  <TableCell>
                    <IconButton onClick={() => handleAddToFavorite(country)}>
                      <FavoriteIcon
                        color={
                          isCountryFavorite(country) ? 'secondary' : 'action'
                        }
                      />
                    </IconButton>
                  </TableCell>
